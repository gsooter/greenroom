...
def test_save_event_returns_201_when_new(
    ...
    resp = client.post(f"/api/v1/events/{uuid.uuid4()}/save", headers=hdrs())
...
def test_unsave_event_returns_204(
    ...
    resp = client.delete(f"/api/v1/events/{uuid.uuid4()}/save", headers=hdrs())
...
def test_list_saved_events_caps_per_page(
    ...
    client, _u, hdrs = authed_client
    resp = client.get("/api/v1/me/saved-events?per_page=500", headers=hdrs())
    assert resp.status_code == 422
